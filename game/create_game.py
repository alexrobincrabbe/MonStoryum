...
def create_rooms() -> list:
    ...
    # create monsters
    # talk to response
    g_speak = "it is probably swearing at you, but you don't understand "\
        "goblish"
    ...
    goblin_captain = Monster(
        ...
        [],
        g_speak)
    drunk_goblin = Monster("goblin", dg_details, 10, 1, -
                           10, no_armor, dagger, [rusty_key], g_speak)
    # spiders
    spiders = []
    for i in range(5):
        spiders.append(
    ...
    for i in range(3):
        trolls.append(
            Monster("troll", t_details, 20, 2, -4, no_armor, club, [], t_speak)
        )
    # wolves
    wolves = []
    for i in range(2):
    ...
    # create features
    wooden_chest_0 = Feature(
        "wooden chest",
        "the chest is rickety and smells damp",
        [healing_potion],
        False)
    bronze_chest = Feature(
        "bronze chest",
        "the chest is dusty",
        [healing_potion, strength_potion, agility_potion],
        True,
    )
    silver_chest = Feature(
        "silver chest",
        "the chest is smooth and shiny",
        [silver_sword, healing_potion],
        True,
    )
    golden_chest = Feature(
        "golden chest",
        "the chest has strange markings on it",
        [dragonscale_armor, super_healing_potion, super_healing_potion],
        True,
    )
    ...
    wooden_chest = Feature(
        "wooden chest",
        "goblins like to store their stuff in chests",
        [super_healing_potion],
        False,
    )
    table = Feature(
        "table", "it has goblin brew stains all over it", [silver_key], False
    )
    bag_of_potions = Feature(
        "bag of potions",
        "someone just left this lying around here",
...
def create_armor() -> tuple:
    ...
    leather_armor = Armor(
        "leather armor", "it is light, and offers some protection", 3, 0
    )
    plate_armor = Armor(
        "plate armor", "it is heavy, but offers good protection", 5, -1)
...
def create_potions() -> tuple:
    ...
    healing_potion = Potion(
        "healing potion",
        "it is red and smells fruity",
        "hp",
        10)
    super_healing_potion = Potion(
        "super healing potion", "really potent stuff", "hp", 20
    )
    agility_potion = Potion(
        "agility potion",
        "it is green and sticky",
        "agility",
        3)
    strength_potion = Potion(
        ...
        "strength",
        2)
    return healing_potion, super_healing_potion, agility_potion, \
        strength_potion
...
def create_keys() -> tuple:
    ...
    gold_medallion = Key(
        "gold medallion", "it is proof that you killed the dragon", "none"
    )
...
def create_room_descriptions() -> tuple[list, list]:
    ...
    """
    room_descriptions = [
        "Regaining consciousness, you open your eyes and realise you "
        "are in a dark, dank-smelling dungeon. From the faint sounds "
        "above you, you realise you are in the depths of the citadel, "
        "controlled by the evil sorceress queen, Achlys. "
        "It dawns on you that by wrongfully imprisoning you, Achlys "
        "has removed the last obstacle to her malicious schemes. "
        "You must find a way to escape, though you know that her loyal "
        "monsters will fight you to the death. "
        ...
        "look down and see three big white egg sacs. "
        "They seem to be wriggling. "
        "You look up and see thick webs and dark shapes moving hurriedly "
        "across the ceiling. Dozens of glowing, "
        "hungry eyes are fixed on you.",  # 4
        "Climbing a few roughly hewn steps you see ahead of you a shaft "
        "of light shining down upon an old stone well. \n"
        "An anvil sits on a stone pile next to the well and discarded "
        "bottles are strewn about the floor. "
        "A stack of buckets has been knocked over and left where they fell. \n"
        "Goblins and trolls are not renowned for their housekeeping! \n"
        "As you approach, something inside the well catches your eye "
        "- is that metal?",  # 5
        "You find yourself in another cavern, this time it's huge. "
        "You look up and wonder if those are stars you see. \n"
        "You can hear something flying above you, circling - probably  birds, "
        "you say to yourself. Spanning the centre of the cavern is a grand "
        "bridge made of cut stone. Torches line one side and appear to go on "
        "forever, glowing in the darkness, dimly lighting the path."
        "You can't see where it leads - a mighty, thick fog is blocking the "
        "view. Anything could be lurking inside. \n You gingerly edge closer "
        "to the low wall of the bridge and peer over at the blackness below "
        "you. The air feels electric, alive. \nIt occurs to you that this "
        "bridge is incredibly large, even for trolls. "
        "You try not to think about what that might mean, but it is a thought "
        "that sticks with you as you continue on your way.\n \n"
        "You suddenly hear a slow, rhythmic sound and, as you're getting "
        "closer to the source, it's becoming so loud it's making the bridge"
        " rumble. \nIt's like the stone is snoring…you can feel the air"
        " tremble around you as it breathes in and out. A sudden realisation "
        "as the monster comes into sight…the rumours of terrorised citizens"
        " were true. Dragons still exist! \n"
        "You stare at the dragon in amazement as a flicker of fire escapes "
        "its nostrils. \n"
        "Then a huge yellow eye snaps open and gazes back at you.",  # 6
        "You hurry past the dragon, down a passageway. You dash through the "
        "next unlocked door you find and are met by a trio of goblins, "
        "one appears to be in charge. They grunt at you.",  # 7
        "As you turn the corner, you hear guffawing and the snorts of Goblish "
        "being spoken and the grunts of a troll. You see them, quaffing a "
        "dark brew, two goblins and a troll sitting at a table, their drinks "
        "spilling as they see you. They appear startled - and then irritated "
        "- that you've disturbed their fun.",  # 8
        "The room is deadly silent but you are unnerved by the feeling that "
        "you're not alone. Taking a few careful steps forward, you hear the "
        "slow scraping sound of metal against concrete. \n"
        "Two deep growls echo together, bouncing off the walls. They vibrate "
        "through your body and the hair on the back of your neck stands on "
        "end. \nThen you see them, waiting for you. \n"
        "They must have heard you coming…because this goblin and troll have "
        "their weapons ready in-hand….and have a couple of massive, ferocious "
        "wolves guarding them. The hairy beasts raise their hackles…and stare "
        "into your soul, circling, they appear to be starving…saliva drips "
        "from their snarling mouths. They are ready to pounce, just waiting "
        "for the order. \nThe troll lumbers forward as the goblin lets out an "
        "ungodly screeching sound. You see a flash of teeth and claws.",  # 9
        "What is this I see before me? Somebody has left an ornately "
        "embroidered bag full of potions just lying here. It looks very out "
        "of place…perhaps you are going to need these?",  # 10
        "Pushing open a door that is much heavier than the others, daylight "
        "dazzles you. Shielding your eyes, you see a hulking shape ahead of "
        "you, blocking the exit to this hellish place. You have fought so "
        "hard and freedom is so close. A Stone Guardian stomps towards you.",
        # 11
    ]
    ...
    room_descriptions_visited = [
        ...
        "An anvil sits on a stone pile next to the well and discarded bottles "
