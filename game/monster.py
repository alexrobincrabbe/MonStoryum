...
class Monster:
    ...
    def attack(self, target):
        ...
        '''
        time.sleep(0.7)
        print(f'[blue]{self.description}[/blue] '
              f'attacks [blue]{target.description}[/blue]...')

        time.sleep(0.7)
        hit = self.agility + self.weapon.hit + (rnd.random() * 15)
        dodge = target.agility + target.armor.dodge + (rnd.random() * 15)
        if hit > dodge:
            damage = self.strength + rnd.randrange(self.weapon.damage[0],
            ...
            print(f'[blue]{target.description}[/blue] has '
                  f'[green1]{target.hp}[/green1]/'
                  f'[chartreuse4]{target.start_hp}[/chartreuse4] remaining')
    ...
    def talk(self, room):
        ...
        if self.riddle_solved is False:
            print(
                ...
                "[chartreuse4]'I have been here for centuries and you are "
                "the first to say a single word to me. Usually they all "
                "run away! I will let you pass by me unharmed, but only "
                "if you play my game…and win!'[/chartreuse4] ")
            while True:
                answer = Prompt.ask(
                    "[chartreuse4] 'would you like to play?'[/chartreuse4] "
                    "(yes/no)")
                if answer == "yes" or answer == "y":
                    print(
                        " [chartreuse4]'Let us begin, the rules of my game "
                        "are simple. \n"
                        "You must guess the letters for the word I am "
            ...
            while True:
                answer_2 = Prompt.ask(
                    " [chartreuse4]'are you you sure you want "
            ...
            if win is False:
                print(
                    " [chartreuse4]'It has been so long "
                    "since my last meal...'[/chartreuse4] ")
                ...
                print(
                    ...
                    "Taking a deep breath, you brave the bridge and "
                    "scurry past as fast as you can.' ")
        else:
            print(
                "[chartreuse4]'you have already bested my game human. "
                "Leave quickly before I change my mind'[/chartreuse4]")
...
class Player(Monster):
    ...
    def print_weapons(self):
        ...
        damage_data = (
            [f'{weapon.damage[0]}-{weapon.damage[1]}' for weapon
             in self.weapons])
    ...
    def print_potions(self):
        ...
        effect_data = (['{0:+}'.format(potion.effect)
                ...
                if equip_object == item.description:
                    if item.type == "weapon":
                        if self.weapon.description == item.description:
                            print(
                                f"[green]{item.description}[/green] is "
                                "already equipped")
                        else:
                            self.weapon = item
    ...
    def status(self):
        ...
        weapon_table.add_column(
            "Damage", [f'{self.weapon.damage[0]}-{self.weapon.damage[1]}'])
        weapon_table.add_column("Hit", ['{0:+}'.format(self.weapon.hit)])
        console.print(weapon_table, style="red")
        print("Armor value reduces damage taken, Dodge reduces the chance"
              " to be hit")
        ...
        print("Strength increases damage, Agility increases hit and dodge")
